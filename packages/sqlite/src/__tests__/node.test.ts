...
test("node simple", async () => {
  ...
  const res = await pipe(
    client.executeRaw<{ col: number }>("SELECT 1 as col"),
...
test("node create and find", async () => {
  ...
  const res = await pipe(
    ...
    T.chain(() =>
      client.create("events", { id: "some", createdAt: new Date("2020-01-01") })
    ),
  ...
  const schema = Schema.defineSchema({
    tables: {
      events: {
        columns: {
          id: Schema.column({ type: Schema.text(), primaryKey: true }),
          someData: Schema.column({
            type: Schema.json<{ a: "a"; b: { b1: "b1" } }>(),
          }),
        },
      },
    },
...
test("node should create db file", async () => {
  ...
  await pipe(
    client.migrateIfNeeded(),
    T.provideSomeLayer(makeSqliteConnection("test-db", testDbPath)),
    runMain
  );
